...
async def scheduler():
    ...
    if not settings.notification_schedule_enabled:
        log.info("notification_scheduler_disabled")
        return
    stop = asyncio.Event()
        ...
        try:
            with SessionLocal() as db:
                with connector_lock(db, "notification-jobs"):
                    ...
                    db.commit()
                    delivered = await NotificationDeliveryService(db).deliver_pending()
            ...
            log.info("notification_batch_skipped", reason="already_running")
        except Exception as exc:
            log.exception("notification_batch_failed", error_type=type(exc).__name__)
        try:
            await asyncio.wait_for(
                stop.wait(), settings.notification_schedule_interval_minutes * 60
            )
        except TimeoutError:
            pass
