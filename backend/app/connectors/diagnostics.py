...
def diagnose_https(endpoint: str, timeout: float = 15) -> NetworkDiagnostic:
    ...
    tcp_ok = tls_ok = False
    tls_version = cipher = subject = category = error_type = error_message = None
    try:
        started = time.perf_counter()
        infos = socket.getaddrinfo(host, port, type=socket.SOCK_STREAM)
        dns_ms = round((time.perf_counter() - started) * 1000, 2)
        ...
        try:
            ...
            tls_ms = round((time.perf_counter() - started) * 1000, 2)
            tls_ok = True
            tls_version = tls.version()
            cipher = tls.cipher()[0] if tls.cipher() else None
            subject = ", ".join(
                "=".join(x) for group in tls.getpeercert().get("subject", ()) for x in group
            )
            tls.close()
        ...
        category = category or ("tls_timeout" if tcp_ok else "tcp_timeout")
        error_type = type(exc).__name__
        error_message = str(exc)
    except OSError as exc:
        category = category or ("tls_transport" if tcp_ok else "tcp_connect")
        error_type = type(exc).__name__
        error_message = str(exc)
