...
def import_census_geography(db: Session, data_dir: Path | None = None) -> dict:
    ...
    existing = db.scalar(select(GeographyDataset).where(GeographyDataset.version == VERSION))
    if existing:
        return {"dataset_id": existing.id, "districts": 0, "municipalities": 0, "unchanged": True}
    dataset = GeographyDataset(
        name="South African administrative geography",
        source=SOURCE,
    ...
    db.add(dataset)
    db.flush()
    provinces = {p.code: p for p in db.scalars(select(Province))}
    districts = {}
    with open(data_dir / "za_districts_census2022.csv") as f:
        for row in csv.DictReader(f):
            ...
            db.add(district)
            db.flush()
            districts[row["dc_code"]] = district
    count = 0
    with open(data_dir / "za_municipalities_census2022.csv") as f:
        for row in csv.DictReader(f):
            ...
            if not province:
                continue
            metro = row["miif_category"] == "METRO"
            db.add(
                Municipality(
                    dataset_id=dataset.id,
                    district_id=None if metro else districts.get(row["dc_code"]).id,
                    province_id=province.id,
                    code=row["muni_code"],
                    name=row["name"],
                    municipality_type="METROPOLITAN" if metro else "LOCAL",
                    active=True,
                )
            )
            count += 1
