...
from app.schemas.domain import SavedTenderItem,SavedTenderReminderInput,SavedTenderReminderOut,TenderCard,TenderIdsRequest,TenderSavedState
...
def saved_state(tender_id:str,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
 if not db.get(Tender,tender_id):raise ApiError(404,"TENDER_NOT_FOUND","Tender not found")
 item=db.scalar(select(SavedTender).where(SavedTender.user_id==user.id,SavedTender.tender_id==tender_id));return TenderSavedState(tender_id=tender_id,saved=item is not None,closing_reminders_enabled=item.closing_reminders_enabled if item else None,reminder_days=item.reminder_days if item else None)
...
def saved_states(data:TenderIdsRequest,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
 items={item.tender_id:item for item in db.scalars(select(SavedTender).where(SavedTender.user_id==user.id,SavedTender.tender_id.in_(data.tender_ids)))};return [TenderSavedState(tender_id=x,saved=x in items,closing_reminders_enabled=items[x].closing_reminders_enabled if x in items else None,reminder_days=items[x].reminder_days if x in items else None) for x in data.tender_ids]
@router.put("/tenders/{tender_id}/reminders",response_model=SavedTenderReminderOut)
def set_reminders(tender_id:str,data:SavedTenderReminderInput,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
 item=db.scalar(select(SavedTender).where(SavedTender.user_id==user.id,SavedTender.tender_id==tender_id))
 if not item:raise ApiError(404,"SAVED_TENDER_NOT_FOUND","Save the tender before configuring reminders")
 days=sorted(set(data.reminder_days),reverse=True)
 if any(day not in {0,1,3,7} for day in days):raise ApiError(422,"INVALID_REMINDER_DAYS","Supported reminder days are 7, 3, 1 and 0")
 item.closing_reminders_enabled=data.enabled;item.reminder_days=days;db.commit();return SavedTenderReminderOut(tender_id=tender_id,enabled=item.closing_reminders_enabled,reminder_days=item.reminder_days)
