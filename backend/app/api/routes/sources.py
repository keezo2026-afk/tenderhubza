...
def sources(page:int=Query(1,ge=1),page_size:int=Query(20,ge=1,le=100),db:Session=Depends(get_db)):
    return Page(items=list(db.scalars(select(Source).offset((page-1)*page_size).limit(page_size)).all()),page=page,page_size=page_size,total=db.scalar(select(func.count()).select_from(Source)) or 0)
