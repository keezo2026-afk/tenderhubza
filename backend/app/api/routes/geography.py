...
from app.schemas.domain import DistrictOut,MunicipalityOut,ProvinceOut
...
def districts(province_id:str|None=None,db:Session=Depends(get_db)):
    query=select(District)
    if province_id:query=query.where(District.province_id==province_id)
    return list(db.scalars(query.order_by(District.name)).all())
