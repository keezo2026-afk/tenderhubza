...
def register(data: RegisterRequest, request: Request, db: Session = Depends(get_db)):
    limit(request, "register", get_settings().auth_rate_limit_per_minute)
    ...
    user.profile = Profile(
        display_name=f"{data.first_name} {data.last_name}", notification_preferences={}
    )
    db.add(user)
    try:
        db.commit()
...
def login(data: LoginRequest, request: Request, db: Session = Depends(get_db)):
    limit(request, "login", get_settings().auth_rate_limit_per_minute)
    user = db.scalar(select(User).where(User.email == data.email.lower()))
    if not user or not verify_password(data.password, user.password_hash):
        raise ApiError(401, "INVALID_CREDENTIALS", "Email or password is incorrect")
    if user.status != "ACTIVE":
        raise ApiError(403, "ACCOUNT_DISABLED", "Account is not active")
    return _tokens(db, user)


@router.post("/refresh", response_model=TokenResponse)
def refresh(data: RefreshRequest, request: Request, db: Session = Depends(get_db)):
    limit(request, "refresh", get_settings().auth_rate_limit_per_minute)
    digest = token_digest(data.refresh_token)
    record = db.get(RefreshToken, digest)
    ...
    if record.used_at or record.revoked_at:
        ...
        db.commit()
        raise ApiError(401, "REFRESH_TOKEN_REUSE", "Refresh token reuse detected; session revoked")
    if expires <= now:
        raise ApiError(401, "EXPIRED_REFRESH_TOKEN", "Refresh token has expired")
    user = db.get(User, record.user_id)
    if not user or user.status != "ACTIVE":
        raise ApiError(401, "INVALID_USER", "User is unavailable")
    record.used_at = now
    result = _tokens(db, user, record.family_id)
    record.replaced_by_digest = token_digest(result.refresh_token)
    db.commit()
    return result
...
def logout(
    ...
    expires = datetime.fromtimestamp(payload["exp"], UTC)
    db.add(RevokedToken(digest=token_digest(credentials.credentials), expires_at=expires))
...
async def request_reset(
    ...
    settings = get_settings()
    limit(request, "password-reset", settings.auth_rate_limit_per_minute)
    user = db.scalar(select(User).where(User.email == data.email.lower()))
    token = None
    if user:
        ...
        db.commit()
        reset_url = f"{settings.public_app_url}?token={token}"
        await email_provider(settings).send_password_reset(user.email, reset_url)
    return PasswordResetRequested(
        development_token=token
        if settings.environment == "development"
        and settings.mail_adapter == "development"
        and settings.expose_development_reset_token
        else None
    )
...
def confirm_reset(data: PasswordResetConfirm, request: Request, db: Session = Depends(get_db)):
    limit(request, "password-reset-confirm", get_settings().auth_rate_limit_per_minute)
    record = db.get(PasswordResetToken, token_digest(data.token))
    now = datetime.now(UTC)
    if not record:
    ...
    if record.used_at or expires <= now:
        raise ApiError(400, "INVALID_RESET_TOKEN", "Password reset token is invalid or expired")
    user = db.get(User, record.user_id)
