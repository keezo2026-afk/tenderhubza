...
class FixtureConnector(ETendersConnector):
 def __init__(self,releases):self.releases=releases;self.page_number=1;self.page_size=100;self.date_from=None;self.date_to=None;self.timeout=1;self.max_attempts=1;self._client=None
 async def discover(self):return [DiscoveredItem(r["ocid"],f"https://ocds-api.etenders.gov.za/api/OCDSReleases/release/{r['ocid']}",r.get("id")) for r in self.releases]
 async def fetch(self,item):
  release=next(r for r in self.releases if r["ocid"]==item.source_identifier);digest=hashlib.sha256(json.dumps(release,sort_keys=True,separators=(",",":")).encode()).hexdigest();return RawPayload(item.source_identifier,item.url,copy.deepcopy(release),datetime.now(timezone.utc),self.VERSION,200,"application/json",item.url,digest,release.get("id"))
def source(db):
...
async def test_ocds_normalization_maps_without_inventing_data():
 normalized=await ETendersConnector().normalize(FIXTURE[0]);assert normalized.reference_number=="ZNT-CONSTRUCTION-001";assert normalized.closing_time.hour==11;assert normalized.currency=="ZAR";assert normalized.municipality is None;assert len(normalized.documents)==1
@pytest.mark.asyncio
async def test_connector_retries_transient_response(monkeypatch):
 ...
 def handler(request):
  ...
  if calls==1:return httpx.Response(503,request=request)
 ...
 assert calls==2;assert items[0].source_identifier==FIXTURE[0]["ocid"]
def test_refresh_rotation_reuse_detection_and_password_reset(client,user_payload):
 client.post("/api/v1/auth/register",json=user_payload);login=client.post("/api/v1/auth/login",json={"email":user_payload["email"],"password":user_payload["password"]});assert login.status_code==200;old=login.json()["refresh_token"]
 rotated=client.post("/api/v1/auth/refresh",json={"refresh_token":old});assert rotated.status_code==200;assert rotated.json()["refresh_token"]!=old
 reuse=client.post("/api/v1/auth/refresh",json={"refresh_token":old});assert reuse.status_code==401;assert reuse.json()["error"]["code"]=="REFRESH_TOKEN_REUSE"
...
def test_error_envelope(client):
 response=client.post("/api/v1/auth/register",json={"email":"bad","password":"x"});assert response.status_code==422;assert set(response.json()["error"])=={"code","message","details"}
def test_authoritative_geography_import_is_idempotent(db):
 for code,name in [("EC","Eastern Cape"),("FS","Free State"),("GP","Gauteng"),("KZN","KwaZulu-Natal"),("LP","Limpopo"),("MP","Mpumalanga"),("NC","Northern Cape"),("NW","North West"),("WC","Western Cape")]:db.add(Province(code=code,name=name))
 db.commit();result=import_census_geography(db);assert result["districts"]==52;assert result["municipalities"]==213;assert import_census_geography(db)["unchanged"] is True;assert db.scalar(select(GeographyDataset)).source.startswith("Statistics South Africa")
def test_search_filters_dates_and_pagination(client,db):
 src=source(db);kzn=db.scalar(select(Province).where(Province.code=="KZN"));
 for i in range(5):db.add(Tender(source_id=src.id,source_reference=f"ref-{i}",title=("Construction project" if i%2==0 else "Office supplies"),organisation="Public Works",province="KwaZulu-Natal",province_id=kzn.id,source_url=f"https://example.gov.za/{i}",status="OPEN",issue_date=date(2026,8,10+i),closing_date=date(2026,9,10+i)))
 db.commit();result=client.get(f"/api/v1/tenders?q=construction&province_id={kzn.id}&closing_from=2026-09-10&closing_to=2026-09-30&page=1&page_size=2");assert result.status_code==200;body=result.json();assert body["total"]==3;assert body["total_pages"]==2;ids1={x["id"] for x in body["items"]};ids2={x["id"] for x in client.get(f"/api/v1/tenders?q=construction&province_id={kzn.id}&page=2&page_size=2").json()["items"]};assert not ids1&ids2
