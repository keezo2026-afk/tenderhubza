...
@Composable fun AppNavigation(vm:AuthViewModel,auth:za.co.tenderhub.data.repository.AuthRepository,tenders:TenderRepository){val nav=rememberNavController();val state by vm.state.collectAsState();val navigateMain:(String)->Unit={nav.navigate(it){launchSingleTop=true}};LaunchedEffect(state){when(state){is AuthState.Authenticated->nav.navigate(Routes.Home){popUpTo(0)};AuthState.Anonymous->if(nav.currentDestination?.route !in setOf(Routes.Welcome,Routes.Login,Routes.SignUp,Routes.Forgot))nav.navigate(Routes.Welcome){popUpTo(0)};else->Unit}}
 NavHost(nav,Routes.Splash){
  composable(Routes.Splash){SplashScreen()};composable(Routes.Welcome){WelcomeScreen({nav.navigate(Routes.Login)},{nav.navigate(Routes.SignUp)})};composable(Routes.Login){LoginScreen(state,vm::login,{nav.navigate(Routes.Forgot)},{nav.navigate(Routes.SignUp)})};composable(Routes.SignUp){SignUpScreen(state,vm::register)};composable(Routes.Forgot){val reset:PasswordResetViewModel=viewModel(factory=factory{PasswordResetViewModel(auth)});val resetState by reset.state.collectAsState();ForgotScreen(resetState,{reset.request(it)},{token,password->reset.confirm(token,password)}){nav.navigate(Routes.Login){popUpTo(Routes.Forgot){inclusive=true}}}}
  ...
  composable(Routes.Search){val search:SearchViewModel=viewModel(factory=factory{SearchViewModel(tenders)});val data by search.state.collectAsState();val geo by search.geography.collectAsState();MainShell(Routes.Search,navigateMain){padding->androidx.compose.foundation.layout.Box(Modifier.padding(padding)){SearchDataScreen(data,search.filters,geo,{q,f,s->search.search(q,f,s)},{search.search()},{search.loadMore()}){nav.navigate(Routes.detail(it))}}}}
