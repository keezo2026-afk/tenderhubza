...
@Composable fun SavedSearchesScreen(state:DataState<List<SavedSearch>>,onRetry:()->Unit,onRun:(SavedSearch)->Unit,onRename:(SavedSearch,String)->Unit,onDelete:(SavedSearch)->Unit){
...
@Composable fun AccountMenuScreen(onProfile:()->Unit,onSaved:()->Unit,onSearches:()->Unit,onSecurity:()->Unit,onLogout:()->Unit){Column(Modifier.fillMaxSize().padding(24.dp)){Text("Account",style=MaterialTheme.typography.headlineLarge);MenuButton("Profile",onProfile);MenuButton("Saved tenders",onSaved);MenuButton("Saved searches",onSearches);MenuButton("Business profile — NOT IMPLEMENTED",{});MenuButton("Notification settings — NOT IMPLEMENTED",{});MenuButton("Subscription — NOT IMPLEMENTED",{});MenuButton("Password & security",onSecurity);Button(onLogout){Text("Log out")}}}
