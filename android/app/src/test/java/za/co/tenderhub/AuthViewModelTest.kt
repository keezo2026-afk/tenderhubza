...
@OptIn(ExperimentalCoroutinesApi::class) class AuthViewModelTest{
 ...
 class FakeRepo:AuthRepository{override val invalidations=kotlinx.coroutines.flow.MutableSharedFlow<Unit>();override suspend fun restore()=ApiResult.Error("No session",401);override suspend fun login(email:String,password:String)=ApiResult.Error("API error",500);override suspend fun register(email:String,password:String,first:String,last:String)=ApiResult.Error("API error");override suspend fun logout(){};override suspend fun requestPasswordReset(email:String)=ApiResult.Error("API error");override suspend fun confirmPasswordReset(token:String,password:String)=ApiResult.Error("API error")}
}
